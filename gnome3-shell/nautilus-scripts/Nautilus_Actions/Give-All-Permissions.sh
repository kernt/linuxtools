...
for ARCHIVE_FULLPATH in $NAUTILUS_SCRIPT_SELECTED_FILE_PATHS; do
    ...
    gnome-terminal -x sudo chmod 777 -R "$ARCHIVE_FULLPATH"
    notify-send -t 5000 -i /usr/share/icons/gnome/32x32/status/info.png "Making Executable Of File(s)/Folder(s) Finished"
done
