#!/bin/bash
# Author : Cezar Matei
process=`ps -u $USER | zenity --list --column "Please select a process to pause:" --text="List all $USER processes:" --width 450 --height 570`
	...
	then
		...
		kill -STOP $select
			...
			select1=`echo $process | cut -d":" -f3  | cut -d" " -f2`
 			echo | zenity --info --text="Process '$select1' with pid '$select' was paused!" --width 400 --height 150
			else
